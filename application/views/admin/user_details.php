                              ...
                              <table id="simple-table" class="table table-striped table-bordered table-hover">
                                 <thead>
                                    <tr>
                                       ...
                                       <th class="center bigger-120">User</th>
                                       <th class="center bigger-120">Email</th>
                                       <th class="center bigger-120">Address</th>
                                       <!-- <th class="center bigger-120">Password</th> -->
                                       <th class="center bigger-120">Image</th>
                                       <th class="center bigger-120">IP</th>
                                       <th class="center bigger-120">Web Browser</th>
...
<script>
  ...
  {
   $.ajax({
      url: '<?php echo site_url('admin/user_con/get_data/') ?>'+page,
      type: 'post',
        ...
        var ob=JSON.parse(data);
         ...
         $.each(ob.user_table,function(key,value){
            html += '<tr align="center">'+
                  ...
                  '<td style="line-height: 50px;">'+ value.user_id + '</td>'+
                  '<td style="line-height: 50px;">'+ value.name + '</td>'+
                  '<td style="line-height: 50px;">'+ value.email + '</td>'+
                  '<td style="line-height: 50px;">'+ value.address + '</td>'+
                  '<td style="line-height: 50px;"><img style="border-radius:30%;" src="<?php echo base_url("upload/user/");?>'+value.image+'" height="70" width="70"></td>'+
                  '<td style="line-height: 50px;">'+ value.ip + '</td>'+
                  '<td style="line-height: 50px;">'+ value.webbrowser + '</td>'+
                  '<td style="line-height: 50px;"><span class="statustag label label-lg label-inverse arrowed-in">'+ value.active + '</span></td>'+
                  '<td>'+
                            '<button class="approve btn btn-primary" style="border-radius: 30%;margin-right: 5px;" value="'+value.user_id+'">Approve</button>'+
                            '<button class="block btn btn-danger" style="border-radius: 30%;" value="'+value.user_id+'">Block</button>'+
                        '</td>'+
                  '</tr>';
         });
  ...
  $(document).on("click", ".pagination li a", function(event){
   ...
   {  
      ...
      $(document).on('click','.approve',function(){
         ...
         var page=$("#cpage").val();
         $.ajax({
            url: '<?php echo site_url('admin/user_con/approve_user/');?>' + uid,
      ...
      $(document).on('click','.block',function(){
         ...
         var page=$("#cpage").val();
         $.ajax({
            url: '<?php echo site_url('admin/user_con/block_user/');?>' + uid,
            success: function(resp){
               $('#showdata').html(resp.view);
               $('#pagination').html(resp.pagination);
               show_admin(page);
